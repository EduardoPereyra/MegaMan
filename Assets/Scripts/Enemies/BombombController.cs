            ...
            switch (bombombType)
            {
                case BombombTypes.Launcher:
                    ...
                    if (launchTimer <= 0)
                    {
                        ...
                        bomb.GetComponent<BombombController>().EnableAI(enableAI);
                        bomb.GetComponent<BombombController>().SetColor(bombombColor);
                        bomb.GetComponent<BombombController>().SetCollideWithTags(collideWithTags);
                        bomb.GetComponent<BombombController>().SetExplosionDamage(enemyController.explosionDamage);
                        bomb.GetComponent<BombombController>().SetShrapnelVectors(shrapnelVectors);
                        bomb.GetComponent<Rigidbody2D>().linearVelocity = bombVelocity;
                ...
                case BombombTypes.Bomb:
                    ...
                    if (rb2d.linearVelocity.y <= -1.0f)
                    {
                        // create explosion animation
                        GameObject explodeEffect = Instantiate(enemyController.explosionPrefab);
                        explodeEffect.name = enemyController.explosionPrefab.name;
                        ...
                        for (int i = 0; i < shrapnel.Length; i++)
                        {
                            ...
                            shrapnel[i].GetComponent<BombombController>().EnableAI(enableAI);
                            shrapnel[i].GetComponent<BombombController>().SetColor(bombombColor);
                            shrapnel[i].GetComponent<BombombController>().SetCollideWithTags(collideWithTags);
                            shrapnel[i].GetComponent<BombombController>().SetExplosionDamage(enemyController.explosionDamage);
                            shrapnel[i].GetComponent<Rigidbody2D>().linearVelocity = shrapnelVectors[i];
                        }
