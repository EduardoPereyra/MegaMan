...
public class MetallController : MonoBehaviour
{
    ...
    private void ShootBullet()
    {
        ...
        for (int i = 0; i < bullets.Length; i++)
        {
            ...
            bullets[i].GetComponent<Bullet>().SetBulletType(Bullet.BulletType.MiniOrange);
            bullets[i].GetComponent<Bullet>().SetDamage(enemyController.bulletDamage);
            bullets[i].GetComponent<Bullet>().SetSpeed(enemyController.bulletSpeed);
            bullets[i].GetComponent<Bullet>().SetDirection(bulletVectors[i]);
            bullets[i].GetComponent<Bullet>().SetCollideWithTags(new string[] {"Player"});
            bullets[i].GetComponent<Bullet>().SetDestroyDelay(5f);
            bullets[i].GetComponent<Bullet>().Shoot();
        }
