...
public class GameManager : MonoBehaviour
{
    ...
    private void RepositionEnemies()
    {
        ...
        foreach (GameObject enemy in enemies)
        {
            ...
            if (enemy.transform.position.y < worldViewCoords.Bottom)
            {
                switch (enemy.name)
                {
                    case "BigEye":
                        ...
                        enemy.transform.position = new Vector3(worldViewCoords.Right - 2.5f, 2f, enemy.transform.position.z);
                        break;
                }
            }
            ...
            if (enemy.transform.position.x < worldViewCoords.Left)
            {
                ...
                {
                    case "Mambu":
                        enemy.transform.position = new Vector3(worldViewCoords.Right, UnityEngine.Random.Range(-0.9f, 0.7f), enemy.transform.position.z);
                        break;
                    case "KillerBomb":
                        enemy.transform.position = new Vector3(worldViewCoords.Right, UnityEngine.Random.Range(-1.5f, 1.5f), enemy.transform.position.z);
                        enemy.GetComponent<KillerBombController>().ResetFollowingPath();
                        break;
                    case "Pepe":
                        enemy.transform.position = new Vector3(worldViewCoords.Right, UnityEngine.Random.Range(-1f, 1f), enemy.transform.position.z);
    ...
    private ItemsController.ItemType PickRandomBonusItem()
    {
        /*
        ...
        ItemsController.ItemType[] items = {
            ItemsController.ItemType.Nothing,
            ItemsController.ItemType.BonusBall,
            ItemsController.ItemType.WeaponEnergySmall,
            ItemsController.ItemType.LifeEnergySmall,
            ItemsController.ItemType.WeaponEnergyBig,
            ItemsController.ItemType.LifeEnergyBig,
            ItemsController.ItemType.ExtraLife,
            ItemsController.ItemType.Nothing,
            ItemsController.ItemType.BonusBall
        };
    ...
    public GameObject GetBonusItem(ItemsController.ItemType itemType)
    {
        GameObject bonusItem = null;

        // pick a random bonus item
        if (itemType == ItemsController.ItemType.Random)
        {
        ...
        switch (itemType)
        {
            case ItemsController.ItemType.Nothing:
                bonusItem = null;
                break;
